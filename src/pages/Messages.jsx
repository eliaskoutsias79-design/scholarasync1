          ...
          <div className="chat-messages">
            ...
            {classMessages.map(m => (
              <div key={m.id} className={`chat-bubble ${m.sender_id === session.user.id ? "mine" : "theirs"}`}>
                <span className="bubble-name" translate="no">{m.sender?.full_name || "Unknown"} {m.sender?.role === "teacher" ? "👩‍🏫" : m.sender?.role === "student" ? "🎓" : "🛡️"}</span>
                <div className="bubble-text" translate="no">{m.content}</div>
                ...
                <div className="student-list">
                  ...
                  {filteredStudents.map(s => (
                    <div key={s.id} className={`student-item ${selectedStudent?.id === s.id ? "selected" : ""}`} onClick={() => setSelectedStudent(s)}>
                      <strong translate="no">{s.full_name || s.email}</strong>
                      <small translate="no">{s.user_class}</small>
                    </div>
                  ))}
                </div>
              </div>
                ...
                {!selectedStudent ? (
                  ...
                  <>
                    <div className="dm-header">
                      <strong translate="no">{selectedStudent.full_name || selectedStudent.email}</strong>
                      <small translate="no">{selectedStudent.user_class}</small>
                    </div>
                    <div className="chat-messages">
      ...
      <div className="dm-sidebar">
        ...
        {conversations.map(c => (
          <div key={c.id} className={`student-item ${selectedConvo?.id === c.id ? "selected" : ""}`} onClick={() => setSelectedConvo(c)}>
            <strong translate="no">{c.full_name || c.email}</strong>
            <small translate="no">{c.role}</small>
          </div>
        ))}
      </div>
      <div className="dm-chat">
        {!selectedConvo ? (
          ...
          <>
            <div className="dm-header"><strong translate="no">{selectedConvo.full_name || selectedConvo.email}</strong><small translate="no">{selectedConvo.role}</small></div>
            <div className="chat-messages">
