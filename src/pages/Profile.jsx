...
export default function ProfileView({
  ...
  const initial = (profileDraft.fullName || profile?.email || "S")
    .charAt(0)
    .toUpperCase();
  const previewAvatarUrl = toSafeHttpUrl(profileDraft.avatarUrl);
      ...
      <section className="profile-hero-card">
        ...
        <div className="profile-identity">
          <div className="profile-avatar-large">
            {previewAvatarUrl ? (
              <img
                ...
                alt={`${profileDraft.fullName || "User"} profile`}
          ...
          <div className="profile-identity-copy">
            ...
            <h2 translate="no">{profileDraft.fullName || "ScholarAsync User"}</h2>
            <p translate="no">{session?.user?.email}</p>
          </div>
        </div>
      </section>
        ...
        <aside className="profile-summary-card">
          ...
          <div className="profile-preview-avatar">
            {previewAvatarUrl ? (
              <img
                src={previewAvatarUrl}
                alt={`${profileDraft.fullName || "User"} profile preview`}
                referrerPolicy="no-referrer"
              />
            ) : (
              <span>{initial}</span>
          ...
          <h3 translate="no">{profileDraft.fullName || "ScholarAsync User"}</h3>
          <span className="profile-preview-role">{profile?.role || "Member"}</span>

          <p translate={profileDraft.bio ? "no" : "yes"}>
            {profileDraft.bio ||
          ...
          <div className="profile-detail-list">
            <div>
              <span>Email</span>
              <strong translate="no">{session?.user?.email}</strong>
            </div>
