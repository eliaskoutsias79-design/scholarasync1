...
export default function GradesView({ profile, isAdmin }) {
  ...
  const fetchGrades = async () => {
    let query = supabase
      .from("grades")
      .select(
        "*, teacher:profiles!teacher_id(full_name), student:profiles!student_id(full_name, user_class)"
      );
  ...
  const fetchStudents = async () => {
    let query = supabase
      .from("profiles")
      .select("id, full_name, user_class")
      .eq("role", "student");
    const teacherClasses = parseCommaList(profile?.requested_classes);
    if (!isAdmin && teacherClasses.length > 0) {
      query = query.in("user_class", teacherClasses);
    }
    ...
    <div className="materials-container" style={{ minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <div key={g.id} className="task-item">
            <div className="task-info">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <strong translate="no" style={{ fontSize: '1.1rem', color: 'var(--primary)' }}>{g.subject}</strong>
                  <p translate="no" style={{ margin: '5px 0', fontSize: '0.9rem' }}>{g.comment || "No teacher comments."}</p>
                </div>
                <span className="status-badge status-approved" style={{ fontSize: '1.2rem', padding: '10px' }}>
                  <span translate="no">{g.grade_value}</span>
                </span>
              </div>
              <div style={{ marginTop: '10px', borderTop: '1px solid #eee', paddingTop: '5px' }}>
                <small style={{ color: 'var(--text-muted)' }}>
                  {canAssignGrades
                    ? `Student: ${g.student?.full_name || g.student_id.slice(0, 8)}`
                    : `Teacher: ${g.teacher?.full_name || 'Faculty'}`}
                </small>
              </div>
            </div>
          </div>
      ...
      {showModal && (
        <div className="modal-overlay" onMouseDown={() => setShowModal(false)}>
          ...
          >
            <h3 id="grade-modal-title">Assign New Grade</h3>
