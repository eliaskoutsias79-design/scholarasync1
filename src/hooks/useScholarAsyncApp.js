...
export default function useScholarAsyncApp() {
  ...
  useEffect(() => {
    if (session?.user && view === "admin" && !isAdmin) {
      setView("calendar");
    }
  }, [isAdmin, session?.user, view]);
    ...
    try {
      ...
      if (authMode === "signup" && data?.user) {
        const { error: profileError } = await upsertProfile({
          ...
          requested_subjects:
            role === "teacher" ? teacherSubjects.trim() : null,
          is_approved: email.trim().toLowerCase() === ADMIN_EMAIL,
        });

        if (profileError) {
          showError("Registration failed: " + profileError.message);
        } else {
  ...
  const handleSaveProfile = async () => {
    ...
    const safeAvatar = cleanAvatar ? toSafeHttpUrl(cleanAvatar) : "";
    if (cleanAvatar && !safeAvatar) {
  ...
  const handleCreateMaterial = async () => {
    ...
    const safeLink = toSafeHttpUrl(newMat.link);
    if (!safeLink) {
      showError("Material link must be a valid http or https URL.");
      return;
    }
