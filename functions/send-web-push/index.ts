...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (table === "messages") {
      ...
      if (record.is_class_chat && record.class_name) {
        const [students, teachers] = await Promise.all([
          ...
          supabase
            ...
            .select("id")
            .eq("role", "teacher")
            .ilike("requested_classes", `%${record.class_name}%`)
            .neq("id", record.sender_id),
        ]);
        if (students.error) throw students.error;
        if (teachers.error) throw teachers.error;
        students.data?.forEach(({ id }) => recipients.add(id));
        teachers.data?.forEach(({ id }) => recipients.add(id));
      }
    } else if (table === "announcements") {
      title = notificationText(record.title, "New announcement", 90);
      body = notificationText(
        record.content,
      ...
      body = notificationText(record.content, "A group has new activity.");
    ...
    } else if (table === "personal_assignment_reminders") {
      title = notificationText(
        `Deadline reminder${record.subject ? ` · ${record.subject}` : ""}`,
        "Deadline reminder",
        90
      );
      body = notificationText(record.title, "A deadline is coming up.");
      url = "/?view=calendar";
